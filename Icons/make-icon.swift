...
guard args.count >= 2 else { FileHandle.standardError.write(Data("usage: make-icon.swift <out.png> [size]\n".utf8)); exit(1) }
let outPath = args[1]
...
let bg = CGPath(roundedRect: rect, cornerWidth: scale(226), cornerHeight: scale(226), transform: nil)
ctx.saveGState()
...
let rOut = scale(334), rIn = scale(206)
...
ctx.fillEllipse(in: CGRect(x: cx - rIn + scale(2), y: cy - rIn + scale(2),
